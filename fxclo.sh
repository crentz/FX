...
DEFAULT_ISO_NAME="debian-live-clone.iso"
DEFAULT_SYSTEM_NAME="debian-live-clone"
...
SYSTEM_NAME=$(whiptail --inputbox "Enter the name for the cloned Debian system:" 8 60 "$DEFAULT_SYSTEM_NAME" --title "System Name" 3>&1 1>&2 2>&3)

# Set the ISO destination
ISO_DESTINATION="/path/to/save/$ISO_NAME"

# Set the target directory for rsync
RSYNC_TARGET_DIR="/tmp/debian-rsync-target"
...
show_loading() {
  (
    echo "XXX"
    echo "10"
    echo "Cloning Debian system..."
    sleep 1

    # Use rsync to clone the Debian system
    ...
    LIVE_ISO_DIR="/tmp/debian-live-iso"
